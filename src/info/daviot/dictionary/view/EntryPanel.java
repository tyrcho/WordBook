...
public class EntryPanel extends JPanel {
	...
	public EntryPanel(DictionaryFrame frame) {
		...
		fieldsValidator.putValidator(TRANSLATIONS_FIELD, new NotEmptyValidator(
				"La traduction doit �tre remplie"));
	...
	protected void okButtonClicked() {
		try {
			...
			if (word.length() > 0 && translations.length() > 0) {
				modified = false;
				dictionaryFrame.addEntry(previousWord, word, translations,
						explainationTextArea.getText());
				dictionaryFrame.setModified(true);
				dictionaryFrame.newButtonClicked();
			}
		} catch (FailedValidationException e) {
			new ErrorMessageDialog(dictionaryFrame, "Erreur dans les champs",
