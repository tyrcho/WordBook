...
public class FileManager {
    ...
    private void newClicked() {
        if (!isSaved("Attention aux données en cours", "Sauver les données en cours ?")) return;
    ...
    private boolean saveAsClicked() {
        ...
        if (JFileChooser.APPROVE_OPTION == fileChooser.showSaveDialog(parent)) {
            ...
            if (!file.exists() || JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(parent, "Ecraser le fichier " + file.getAbsolutePath(), "Ecraser ?", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE)) { 
                ...
                if (saved) {
                    setCurrentFile(file);
                    JOptionPane.showMessageDialog(parent, "Le fichier "+currentFile+" a été enregistré.");
                    setModified(false);
                }
                return saved;
            }
    ...
    private void updateStatus() {
        statusBar.setText((currentFile == null ? "" : currentFile.getName()) + " " + (isModified() ? "modifié" : ""));
    }
